...
def memcached_amplification(target, duration, monitor=None):
    ...
    servers = amplification_hunter.get_memcached_servers(limit=50)
    if not servers:
        try:
            with open('txt/memcached_servers.txt', 'r') as f:
                servers = [line.strip() for line in f if line.strip()]
        except:
            add_system_log("[red]ERROR:[/] No memcached servers available (API failed and file not found).")
            return
...
def ssdp_amplification(target, duration, monitor=None):
    ...
    # Try API first, fallback to static file
    servers = amplification_hunter.get_ssdp_servers(limit=50)
    if not servers:
        try:
            with open('txt/ssdp_servers.txt', 'r') as f:
                servers = [line.strip() for line in f if line.strip()]
        except:
            add_system_log("[red]ERROR:[/] No SSDP servers available (API failed and file not found).")
            return
...
def dns_amplification(target, duration, monitor=None):
    ...
    # Try API first, fallback to static file
    servers = amplification_hunter.get_dns_servers(limit=50)
    if not servers:
        try:
            with open('txt/dns_servers.txt', 'r') as f:
                servers = [line.strip() for line in f if line.strip()]
        except:
            add_system_log("[red]ERROR:[/] No DNS servers available (API failed and file not found).")
            return
...
def ntp_amplification(target, duration, monitor=None):
    ...
    # Try API first, fallback to static file
    servers = amplification_hunter.get_ntp_servers(limit=50)
    if not servers:
        try:
            with open('txt/ntp_servers.txt', 'r') as f:
                servers = [line.strip() for line in f if line.strip()]
        except:
            add_system_log("[red]ERROR:[/] No NTP servers available (API failed and file not found).")
            return
